---
name: apply
description: Apply tasks from an OpenSpec change. Use when the user wants to start implementing, continue implementation, or work through tasks.
license: MIT
...
Apply tasks from an OpenSpec change.
...
4. **Read context files**
   ...
   **Also check for test files:**
   Check if `openspec/changes/<name>/tests/` exists. If yes, read all files inside — use them as additional context when applying each task. If the directory does not exist, proceed normally without it.
...
6. **Apply tasks (loop until done or blocked)**
...
- **Can be invoked after apply requirements are satisfied**: If required artifacts are missing, stop and route back to the right workflow step
