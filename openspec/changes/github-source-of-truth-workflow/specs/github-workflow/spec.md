...
WorkLoop SHALL define predictable naming conventions for issues, OpenSpec changes, branches, commits, and pull requests.

#### Scenario: Change name derived from issue
- **WHEN** a GitHub issue is converted into an OpenSpec change
- **THEN** the change name MUST use kebab-case and preserve a stable connection to the issue or outcome

#### Scenario: Pull request title links outcome
- **WHEN** a PR closes or fixes an issue
- **THEN** the PR title or body MUST include the appropriate GitHub issue reference syntax
...
### Requirement: Future automation boundary
WorkLoop SHALL explicitly separate the GitHub source-of-truth workflow contract from later automation, skill behavior changes, GitHub Actions, and template migration.

#### Scenario: Docs and spec only change
- **WHEN** this change is implemented
- **THEN** it MUST NOT require GitHub automation, skill behavior changes, GitHub Actions changes, or issue template migration to satisfy its acceptance criteria
