...
#### Scenario: Scenario files remain compatible with `/code`
- **WHEN** scenario-based test files are generated under `openspec/changes/<name>/tests/`
- **THEN** they remain available for `/code` to read as implementation context without requiring a separate handoff format
