...
### Requirement: Preserve approved design as `/code` input
The system SHALL keep the approved `design.md` in the change directory so `/code`
...
- **THEN** `design.md` remains stored in the change directory and is ready to be
  consumed by `/code` as part of the implementation context
